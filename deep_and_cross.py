...
# =============================================================================
            ...
            self.losses = tf.nn.sigmoid_cross_entropy_with_logits(labels =self.label,logits=self.out)
...
# =============================================================================
    ...
    def fit_on_batch(self, Xi, Xv, y):
        ...
        loss, opt, loss_summary = self.sess.run((self.loss, self.optimizer,self.loss_summary), feed_dict=feed_dict)
    ...
    def predict(self, Xi, Xv,y):
        #dummy_y = [1] * len(Xi)
        batch_index = 0
        Xi_batch, Xv_batch, y_batch = self.get_batch(Xi, Xv, y, self.batch_size, batch_index)
        y_pred = None
        while(len(Xi_batch)) > 0:
            ...
            batch_out = self.sess.run(self.out1, feed_dict=feed_dict)
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None, y_valid=None,
        ...
        for epoch in range(self.epoch):
            ...
            for i in range(total_batch):
                ...
                loss, loss_summary= self.fit_on_batch(Xi_batch, Xv_batch, y_batch)
                count += 1
                print('loss',loss)
                self.writer.add_summary(loss_summary, count)
            ...
            if has_valid and early_stopping and self.train_termination(self.valid_result):
                break
            self.saver.save(self.sess,'model1/my-model1',global_step = epoch)
